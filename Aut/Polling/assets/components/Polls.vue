<template>
 ...
 <v-dialog
      ...
      >
        <v-layout class=" justify-center wrap row white ">
          ...
          <v-flex md8 sm12 xl8 xl12 class="  justify-center ma" width="75%">
                  <v-form ref="form" v-model="valid" lazy-validation >
                    <v-layout wrap row  class="justify-center"  >
                      <v-flex v-for=" item , key in struct" :key="key" class="flex md3 sm6  wrap row"  :class="{'md12 sm12 xm12 xl8':struct[key].flexable}" v-if="!struct[key].hidden">
                          <v-flex v-if="!struct[key].hidden"></v-flex>
                             ...
                             <v-flex v-if='struct[key].type == "id" '>
                               ...
                               <!-- state text_title -->
                             <v-flex  v-else-if="struct[key].type == 'text_title'">
 ...
 <!-- ----------------------date----------------------------- -->
                            <v-flex   v-else-if="struct[key].type == 'date' " class="DataTime" >
                                <!-- data end -->
...
<!-- -----------------------time--------------------------------- -->
                            <v-flex   v-else-if="struct[key].type == 'time' ">
                                    <!-- time end -->
...
<!-- --------------------text more filed ------------------------------>
                            <v-flex  wrap row v-else-if="struct[key].type =='text' && struct[key].plusable === true" >
                              ...
                              <v-flex  v-else>
                                  <v-flex  md12 wrap row>
                                    ...
                                    <v-flex  md12 wrap row v-for="(option,i) in options.en"  :key="i">
                                    <v-layout  wrap row class="justify-center" v-for="(local,j) in polltrans.locales" :key="local">
                                      <!-- {{local}}{{'  key  '+key}}{{'i'+ i}} -->
                                        <v-flex>
                                        ...
                                        <v-flex md11  wrap row>
                                          <v-text-field md12 name='time'
                                           v-model='options[local][i]'   :rules="polltrans.rule.optionRule"
...
<!-- ------------------text one --------------------->
                            <v-flex   v-else-if="struct[key].type =='text'">
...
<script>
    ...
    class Form {
      ...
      setFieldLocales(local,field , val){
        this[local].set(field,val);
      }
      setFieldLocalesASArray(val){

           let fields  = Object.assign({},val);
           for(let locale in fields){
            console.log(locale);
           this[locale].set('options' , val[locale].options);
          
           }
        
           
           //just for fill option this code temp
            ...
            return {
                ...
                struct : new Struct({
                  ...
                  ,items:()=>{ let array = [];
                    ...
                    if( Array.isArray(this.polltrans.en.options)){
                ...
                options : new Option(this.locales)
        ...
        methods:{
            ...
            getOptions(id,obj){
                   ...
                   this.$http.get('getOptions/' + id).then((response) => {
                    console.log(response.data);
                     ...
                     let vm = this;
                    console.log(this.array_option);

                        this.locales.forEach(val=>{

                          this.polltrans[val].options = '';
                        })

                        
                      let fill_trans = new Temp();

                      for(let index in this.locales)
                      fill_trans.set(this.locales[index] , {'options':[]});  
                     
                     console.log('file', fill_trans);
                      let translations;
                       for( let index   in this.array_option){
                            console.log('index is ',index );
                            // console.log('value is ',this.array_option);
                             // console.log('value is ',vm.array_option[index].translations);
                             // if(index==0)
                              translations = this.array_option[index]['translations']
                             console.log('for1:',this.polltrans['ar'].get('options'));
                             console.log('for1:',this.polltrans['en'].get('options'));
                            for (let i in translations) {

                            // console.log('index of transition is ',i);
                            console.log('trans',translations);
                            // console.log('val is ',translations[i].locale);
                             let val = translations[i].locale;
                             console.log('befor:',this.polltrans[val].get('options'));
                             fill_trans[val].options.push( translations[i].name);

                            }
                            for (let i in translations) {

                             let val = translations[i].locale;
                             this.options[val] = fill_trans[val].options;

                            }
                              console.log(fill_trans);

                            this.polltrans.setFieldLocalesASArray(fill_trans);
                       }

                            // array.push(this.array_option[index].translations.name)
          ...
          //add information to object poll 
            ...
            editPolls(obj){
               ...
               let id = obj.id;
               let canVisitorsVote = obj.canVisitorsVote;
               let canVoterSeeResult = obj.canVoterSeeResult;
               let maxCheck = obj.maxCheck;
                // this.poll.setLocales();
               // console.log('the function is :',this.getOptions(id));
                this.locales.forEach(val=>{
                this.polltrans.setFieldLocales(val,'question',obj[val].question);
                 })
                this.getOptions(id,obj);

               let time_start = this.storTimeAsHour(obj.starts_at);
               let time_end = this.storTimeAsHour(obj.ends_at);
               let starts_at = this.GetFormattedDate(obj.starts_at);
               let ends_at = this.GetFormattedDate(obj.ends_at);
            ...
            storeOption(){
              ...
              let index = this.polltrans.locales[local];
                ...
                this.polltrans[index]['options'] = this.options[index];
              }
            }  ,
         ...
         addOption(key){
          ...
          removeOptionMultilang(key,i){
            for(let local in this.polltrans.locales)
            {
              ...
              // this.poll.options.pop();
             if(this.options[index].length>2)

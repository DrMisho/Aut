    ...
    <v-dialog v-model="dialog" width="800" scrollable>
        ...
        <v-card>
                    ...
                    <v-layout row wrap class="justify-space-around">
                        <v-flex md5 xs12>
                            <v-layout row wrap>
                                ...
                                <v-flex xs12>
                                    <select-input :options="gateways" item-text="name" item-value="id" :label="`Payment Gateway *`" v-model="form.payment_gateway_id"/>
                                </v-flex>
                                <v-flex xs12>
                                    <price-input disabled :currency="form.currency" :label="`Amount *`" v-model="form.credit"/>
                                </v-flex>
                                <v-flex xs12>
                                    <v-text-field :label="`Your Account *`" v-model="form.account"/>
                                </v-flex>
                                <v-flex xs12>
            ...
            <v-card-actions>
                ...
                <v-btn :disabled="disabledBtn" :loading="loading" @click="save" color="primary">{{ $t('app.save') }}</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
</template>
